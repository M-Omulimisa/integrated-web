...
class OtpVerify
{
        ...
        if ($this->willCheck($routeName)) {
            ...
            if ($otp instanceof OneTimePassword) {
                ...
                if ($otp->status == "waiting") {
                    ...
                    // set error message
                    \Session::flash('fail', __("otp.otp_status_invalid"));
    ...
    {
        return isset($_COOKIE["otp_login_verified"]) && str_starts_with($_COOKIE["otp_login_verified"], 'user_id_');
    }
    ...
    {
        $cookie = setcookie("otp_login_verified", "user_id_" . $user_id, time() + config("otp.otp_cookie_timeout"));
    }
    ...
    {
        return str_replace("user_id_", "", $_COOKIE["otp_login_verified"]); // intval();
    }
}
