...
class YoUganda {
    ...
    */
    public function ac_get_msisdn_kyc_info($msisdn)
    {
        ...
        $status = 'PENDING';
        $errorResponseMsg = '';
        $errorCode = $surName = $FirstName = $MiddleName = $phoneStatus = null;
        $payLoad = 'null';

        if (curl_error($response) || $xml_response === false) {
            $payLoad             = curl_error($response);
            $errorResponseMsg   = "Connection Problem";
            $status             = 'FAILED';
            $errorCode          = $responseStatusCode;

            // Log::info(['ResponseError' => $xml_response, 'FailedResponseMsg' => curl_error($response)]);
        } else {  

            if (stripos($xml_response, 'HTTP') !== false && stripos($xml_response, 'xml') !== false) {
                $xml_response = strstr($xml_response, '<xml?');
            }

            // Log::info(['leve1Response' => $xml_response]);

            $validated_response = isValidXML($xml_response); 

            if ($responseStatusCode == 200 && $validated_response) {
                $content = isBlankResponse($xml_response);
                $result = $content >= 0 ? $xml_response : null ;
            }
            else{
                $status = 'FAILED';
                $errorCode = $responseStatusCode;
        ...
        if (!is_null($result)) {
            $processedXML       = processXML($result);
            // Log::info(['processedXML' => $processedXML]);

            $transactionResponse      = $processedXML->Response;
            $status                 = $transactionResponse->Status ?? 'PROCESSED';
            $errorResponseMsg   = '';

            if (isset($transactionResponse->StatusCode) && $transactionResponse->StatusCode == '-9999') {
                $errorResponseMsg   = $transactionResponse->StatusMessage;
            }
            elseif (isset($transactionResponse->AccountInformation->PersonalInformation->Names)) {
                $name           = $transactionResponse->AccountInformation->PersonalInformation->Names;
                // $status = $transactionResponse->AccountInformation->BlacklistStatus;
            ...
            if (isset($transactionResponse->StatusCode) && $transactionResponse->Status == 'ERROR') {
                $errorCode          = $transactionResponse->StatusCode;
                $errorResponseMsg = $transactionResponse->StatusMessage;

                if (stripos($errorResponseMsg, 'does not exist / not configured') !== false) {
                    $status = 'OK';
                    $errorResponseMsg = 'Does not exist / not configured';
                    $phoneStatus = 'NOT FOUND';
                }
                elseif (stripos($errorResponseMsg, 'Error getting account information') !== false) {
                    $status = 'OK';
                    $errorResponseMsg = 'No information found';
                    $phoneStatus = 'NOT FOUND';
                }
                elseif (stripos($errorResponseMsg, 'Specify a valid value') !== false) {
                    $status = 'OK';
                    $errorResponseMsg = 'Invalid phone number';
                    $phoneStatus = 'INVALID';
                }
                elseif (stripos($errorResponseMsg, 'Invalid API Username') !== false || stripos($errorResponseMsg, 'API Password') !== false) {
                    $status = 'FAILED';
        ...
        $phone                  = new Request;
        $phone->status          = (string) $status == 'OK' ? 'SUCCESSFUL' : $status;
        $phone->error_code      = (string) $errorCode;
        $phone->error_message   = (string) $errorResponseMsg;
        $phone->message_payload = (string) $payLoad;
        $phone->surname         = (string) $surName;
        $phone->first_name      = (string) $FirstName;
        $phone->middle_name     = (string) $MiddleName;
        $phone->phone_status    = $phoneStatus;
