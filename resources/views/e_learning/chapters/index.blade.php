            ...
            <div class="card-body">
                <ul class="nav nav-tabs" role="tablist">                    
                    ...
                    @can('list_el_chapters')
                        ...
                        @endif
                        <li class="nav-item"><a class="nav-link active" href="{{ url()->current() }}">Topics</a></li>
                    @endcan
                    @can('add_el_chapters')
                        @if ($course->lecture_type != "Weekly")
                            <li class="nav-item"><a class="nav-link" href="{{ url('e-learning/courses/chapters/'.$course->id.'/create') }}">Add Topics</a></li>
                        @endif
                    @endcan
                    ...
                    @endcan
                    {{-- @can('add_el_lectures')
                        <li class="nav-item"><a class="nav-link" href="{{ url('e-learning/courses/lectures/'.$course->id.'/create') }}">Add Lectures</a></li>
                    @endcan --}}
                    @can('list_el_lectures')
    ...
    <script>
        $(document).ready(function() {
            var oTable = $('#dTable').DataTable({
                ...
                ajax: {
                    url: '{!! route('e-learning.chapters.list') !!}',
                    data: function (d) {
                            d.course = '{{ $course->id }}';
                            d.lecture_type = '{{ $course->lecture_type }}';
                         }
                },
                columns: [
